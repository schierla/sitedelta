...
import * as highlightScriptUtils from "./view/highlightScriptUtils";
import { PageState } from "./view/highlightState";

var openTabRequest: null | {
  url: string;
  tabId: number;
...
function handlePageLoad(tabId: number, url: string) {
  if (
    openTabRequest &&
    openTabRequest.tabId == tabId &&
    url == openTabRequest.url
  ) {
  ...
  chrome.tabs.create({ url: "about:blank" }, async (tab) => {
    ...
    if (changed.length > 0) {
      chrome.tabs.update(tab.id || 0, {
        url: chrome.runtime.getURL("pages.htm"),
      });
      return;
    }
...
var contextMenuListener = async function (
  ...
  if (
    ...
    tab.url &&
    tab.id &&
    (info.menuItemId == menuHighlight().id ||
      info.menuItemId == menuHighlightPage().id)
  ) {
...
var messageListener = function (
  ...
  if (request.command == "notifyLoaded") {
    ...
    highlightScriptUtils
      ...
      .then((xpath) => pageUtils.addInclude(request.url, xpath));
    ...
    if (request.scope == "config") {
      try {
        ...
        transferUtils
          .importConfig(config, configUtils.highlightHiddenFields)
          .then((result) =>
            sendResponse(
    ...
    if (request.scope == "config") {
      transferUtils
        .exportConfig(configUtils.highlightHiddenFields)
