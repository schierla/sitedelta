...
interface StateSelectRegion {
	state: PageState.SELECTREGION;
}

interface StateHighlighted {
	state: PageState.HIGHLIGHTED;
	changes: number;
	current: number;
}

type HighlightState = StateError | StateLoaded | StateSelectRegion | StateHighlighted;
		...
		messageHandler: function (request: Record<string, any>, sender: any, sendResponse: (response: any) => void) {
			if (request.command == "getStatus") {
				...
				if (contentscript.state == PageState.HIGHLIGHTED)
					ret = { state: PageState.HIGHLIGHTED, changes: contentscript.numChanges, current: contentscript.currentChange };
				else
					ret = { state: contentscript.state };
				sendResponse(ret);
				...
				if (contentscript.state == PageState.HIGHLIGHTED) {
					...
					sendResponse({ state: PageState.HIGHLIGHTED, changes: contentscript.numChanges, current: contentscript.currentChange });
				} else {
